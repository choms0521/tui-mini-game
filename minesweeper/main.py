"""Entry point: terminal setup, keyboard input, and the main game loop.

Run with ``python main.py`` (or via the launcher).  The loop blocks on
``term.inkey`` with a short timeout so the UI stays responsive without
burning CPU.
"""
from __future__ import annotations

import random

from blessed import Terminal

import game as G
import render as R

POLL_TIMEOUT = 0.05  # seconds; short enough to feel instant


def _map_key(key) -> str | None:
    """Translate a blessed keystroke into a game action name."""
    if key.name == "KEY_UP":
        return "up"
    if key.name == "KEY_DOWN":
        return "down"
    if key.name == "KEY_LEFT":
        return "left"
    if key.name == "KEY_RIGHT":
        return "right"
    if key.name == "KEY_ENTER":
        return "reveal"

    char = str(key).lower()
    return {
        " ": "reveal",
        "f": "flag",
        "r": "restart",
        "q": "quit",
    }.get(char)


def run() -> None:
    """Run Minesweeper; returns when the player presses q."""
    term = Terminal()
    rng = random.Random()
    state = G.new_game(rng)
    dirty = True

    with term.fullscreen(), term.cbreak(), term.hidden_cursor():
        print(term.home + term.clear, end="", flush=True)
        try:
            while True:
                if dirty:
                    R.draw(term, state)
                    dirty = False

                key = term.inkey(timeout=POLL_TIMEOUT)
                if not key:
                    continue

                action = _map_key(key)
                if action == "quit":
                    break
                elif action == "restart":
                    state = G.restart(rng, state)
                    dirty = True
                elif action == "up":
                    new_state = G.move_cursor(state, -1, 0)
                    if new_state is not state:
                        state = new_state
                        dirty = True
                elif action == "down":
                    new_state = G.move_cursor(state, 1, 0)
                    if new_state is not state:
                        state = new_state
                        dirty = True
                elif action == "left":
                    new_state = G.move_cursor(state, 0, -1)
                    if new_state is not state:
                        state = new_state
                        dirty = True
                elif action == "right":
                    new_state = G.move_cursor(state, 0, 1)
                    if new_state is not state:
                        state = new_state
                        dirty = True
                elif action == "reveal":
                    if not state.game_over and not state.won:
                        new_state = G.reveal(state)
                        if new_state is not state:
                            state = new_state
                            dirty = True
                elif action == "flag":
                    if not state.game_over and not state.won:
                        new_state = G.toggle_flag(state)
                        if new_state is not state:
                            state = new_state
                            dirty = True
        except KeyboardInterrupt:
            pass

    print(term.normal + "Thanks for playing Minesweeper!\n", end="")


if __name__ == "__main__":
    run()
